...
func TopRankPriorityQueueFullQueue(imageResults []ImageResult, k int) []ImageResult {
	start := time.Now()
	...
	// }
	fmt.Println("Time spent creating priorityQueue: ", time.Since(start))
	start = time.Now()
	heap.Init(&priorityQueue)
	fmt.Println("Time spent initializing heap: ", time.Since(start))
	...
	fmt.Println("Time spent getting kth largest element from heap: ", time.Since(start))
	return result
}
